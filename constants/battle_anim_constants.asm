...
; BattleAnimObjects indexes (see data/battle_anims/objects.asm)
	...
	const BATTLE_ANIM_OBJ_CLAMP_2             ; 36
	const BATTLE_ANIM_OBJ_BITE                ; 37
	const BATTLE_ANIM_OBJ_CUT_DOWN_LEFT       ; 38
	const BATTLE_ANIM_OBJ_CUT_DOWN_RIGHT      ; 39
	const BATTLE_ANIM_OBJ_CUT_UP_RIGHT        ; 3a
	const BATTLE_ANIM_OBJ_CUT_LONG_DOWN_LEFT  ; 3b
	const BATTLE_ANIM_OBJ_CUT_LONG_DOWN_RIGHT ; 3c
	const BATTLE_ANIM_OBJ_SOLAR_BEAM_CHARGE   ; 3d
	const BATTLE_ANIM_OBJ_ABSORB_CENTER       ; 3e
	...
	const BATTLE_ANIM_OBJ_EGG                 ; 47
	const BATTLE_ANIM_OBJ_FOCUS               ; 48
	const BATTLE_ANIM_OBJ_BIND1               ; 49
	const BATTLE_ANIM_OBJ_BIND2               ; 4a
	const BATTLE_ANIM_OBJ_LEECH_SEED          ; 4b
	const BATTLE_ANIM_OBJ_SOUND               ; 4c
	const BATTLE_ANIM_OBJ_WAVE                ; 4d
	const BATTLE_ANIM_OBJ_CONFUSE_RAY         ; 4e
	const BATTLE_ANIM_OBJ_LEER                ; 4f
	const BATTLE_ANIM_OBJ_LEER_TIP            ; 50
	const BATTLE_ANIM_OBJ_SCREEN              ; 51
	const BATTLE_ANIM_OBJ_HARDEN              ; 52
	const BATTLE_ANIM_OBJ_CHICK               ; 53
	const BATTLE_ANIM_OBJ_AMNESIA             ; 54
	const BATTLE_ANIM_OBJ_ASLEEP              ; 55
	const BATTLE_ANIM_OBJ_SKULL               ; 56
	const BATTLE_ANIM_OBJ_DIG_SAND            ; 57
	const BATTLE_ANIM_OBJ_DIG_PILE            ; 58
	const BATTLE_ANIM_OBJ_SAND                ; 59
	const BATTLE_ANIM_OBJ_PARALYZED           ; 5a
	const BATTLE_ANIM_OBJ_STRING_SHOT         ; 5b
	const BATTLE_ANIM_OBJ_HAZE                ; 5c
	const BATTLE_ANIM_OBJ_MIST                ; 5d
	const BATTLE_ANIM_OBJ_SMOG                ; 5e
	const BATTLE_ANIM_OBJ_POISON_GAS          ; 5f
	const BATTLE_ANIM_OBJ_HORN                ; 60
	const BATTLE_ANIM_OBJ_NEEDLE              ; 61
	const BATTLE_ANIM_OBJ_PETAL_DANCE         ; 62
	const BATTLE_ANIM_OBJ_SLUDGE_BOMB         ; 63
	const BATTLE_ANIM_OBJ_PAY_DAY             ; 64
	const BATTLE_ANIM_OBJ_SONICBOOM_JP_UNUSED ; 65
	const BATTLE_ANIM_OBJ_MIMIC               ; 66
	const BATTLE_ANIM_OBJ_ATTRACT             ; 67
	const BATTLE_ANIM_OBJ_BONEMERANG          ; 68
	const BATTLE_ANIM_OBJ_BONE_CLUB           ; 69
	const BATTLE_ANIM_OBJ_BONE_RUSH           ; 6a
	const BATTLE_ANIM_OBJ_SWIFT               ; 6b
	const BATTLE_ANIM_OBJ_KINESIS             ; 6c
	const BATTLE_ANIM_OBJ_FLASH               ; 6d
	const BATTLE_ANIM_OBJ_SHINY               ; 6e
	const BATTLE_ANIM_OBJ_SKY_ATTACK          ; 6f
	const BATTLE_ANIM_OBJ_LICK                ; 70
	const BATTLE_ANIM_OBJ_WITHDRAW            ; 71
	const BATTLE_ANIM_OBJ_DRAIN               ; 72
	const BATTLE_ANIM_OBJ_GROWTH              ; 73
	const BATTLE_ANIM_OBJ_CONVERSION2         ; 74
	const BATTLE_ANIM_OBJ_SMOKE               ; 75
	const BATTLE_ANIM_OBJ_SMOKESCREEN         ; 76
	const BATTLE_ANIM_OBJ_SWORDS_DANCE        ; 77
	const BATTLE_ANIM_OBJ_SPEED_LINE          ; 78
	const BATTLE_ANIM_OBJ_SHARPEN             ; 79
	const BATTLE_ANIM_OBJ_DEFENSE_CURL        ; 7a
	const BATTLE_ANIM_OBJ_METRONOME_HAND      ; 7b
	const BATTLE_ANIM_OBJ_METRONOME_SPARKLE   ; 7c
	const BATTLE_ANIM_OBJ_DISABLE             ; 7d
	const BATTLE_ANIM_OBJ_AGILITY             ; 7e
	const BATTLE_ANIM_OBJ_HEART               ; 7f
	const BATTLE_ANIM_OBJ_FLAME_WHEEL         ; 80
	const BATTLE_ANIM_OBJ_SACRED_FIRE         ; 81
	const BATTLE_ANIM_OBJ_COTTON_SPORE        ; 82
	const BATTLE_ANIM_OBJ_MILK_DRINK          ; 83
	const BATTLE_ANIM_OBJ_ANGER               ; 84
	const BATTLE_ANIM_OBJ_HEAL_BELL           ; 85
	const BATTLE_ANIM_OBJ_HEAL_BELL_NOTE      ; 86
	const BATTLE_ANIM_OBJ_BATON_PASS          ; 87
	const BATTLE_ANIM_OBJ_LOCK_ON             ; 88
	const BATTLE_ANIM_OBJ_MIND_READER         ; 89
	const BATTLE_ANIM_OBJ_SAFEGUARD           ; 8a
	...
	const BATTLE_ANIM_OBJ_KARATE_CHOP         ; d0
...
; BattleAnimFrameData indexes (see data/battle_anims/framesets.asm)
	...
	const BATTLE_ANIM_FRAMESET_CLAMP_1                    ; 3a
	const BATTLE_ANIM_FRAMESET_CLAMP_2                    ; 3b
	...
	const BATTLE_ANIM_FRAMESET_FLAMETHROWER_ALT           ; c6
...
; BattleBGEffects indexes (see engine/battle_anims/bg_effects.asm)
	...
	const BATTLE_BG_EFFECT_CYCLE_OBPALS_GRAY_AND_YELLOW_FULL_SHIFT     ; 07
	const BATTLE_BG_EFFECT_CYCLE_MID_OBPALS_GRAY_AND_YELLOW            ; 08
	const BATTLE_BG_EFFECT_CYCLE_BGPALS_INVERTED                       ; 09
	const BATTLE_BG_EFFECT_HIDE_MON                                    ; 0a
	const BATTLE_BG_EFFECT_SHOW_MON                                    ; 0b
	const BATTLE_BG_EFFECT_ENTER_MON                                   ; 0c
	const BATTLE_BG_EFFECT_RETURN_MON                                  ; 0d
	const BATTLE_BG_EFFECT_SURF                                        ; 0e
	const BATTLE_BG_EFFECT_WHIRLPOOL                                   ; 0f
	const BATTLE_BG_EFFECT_TELEPORT                                    ; 10
	const BATTLE_BG_EFFECT_NIGHT_SHADE                                 ; 11
	const BATTLE_BG_EFFECT_BATTLEROBJ_1ROW                             ; 12
	const BATTLE_BG_EFFECT_BATTLEROBJ_2ROW                             ; 13
	const BATTLE_BG_EFFECT_DOUBLE_TEAM                                 ; 14
	const BATTLE_BG_EFFECT_ACID_ARMOR                                  ; 15
	const BATTLE_BG_EFFECT_RAPID_FLASH                                 ; 16
	const BATTLE_BG_EFFECT_FADE_MON_TO_LIGHT                           ; 17
	const BATTLE_BG_EFFECT_FADE_MON_TO_BLACK                           ; 18
	const BATTLE_BG_EFFECT_FADE_MON_TO_LIGHT_REPEATING                 ; 19
	const BATTLE_BG_EFFECT_FADE_MON_TO_BLACK_REPEATING                 ; 1a
	const BATTLE_BG_EFFECT_CYCLE_MON_LIGHT_DARK_REPEATING              ; 1b
	const BATTLE_BG_EFFECT_FLASH_MON_REPEATING                         ; 1c
	const BATTLE_BG_EFFECT_FADE_MONS_TO_BLACK_REPEATING                ; 1d
	const BATTLE_BG_EFFECT_FADE_MON_TO_WHITE_WAIT_FADE_BACK            ; 1e
	const BATTLE_BG_EFFECT_FADE_MON_FROM_WHITE                         ; 1f
	const BATTLE_BG_EFFECT_SHAKE_SCREEN_X                              ; 20
	const BATTLE_BG_EFFECT_SHAKE_SCREEN_Y                              ; 21
	const BATTLE_BG_EFFECT_WITHDRAW                                    ; 22
	const BATTLE_BG_EFFECT_BOUNCE_DOWN                                 ; 23
	const BATTLE_BG_EFFECT_DIG                                         ; 24
	const BATTLE_BG_EFFECT_TACKLE                                      ; 25
	const BATTLE_BG_EFFECT_BODY_SLAM                                   ; 26
	const BATTLE_BG_EFFECT_WOBBLE_MON                                  ; 27
	const BATTLE_BG_EFFECT_REMOVE_MON                                  ; 28
	const BATTLE_BG_EFFECT_WAVE_DEFORM_MON                             ; 29
	const BATTLE_BG_EFFECT_PSYCHIC                                     ; 2a
	const BATTLE_BG_EFFECT_BETA_SEND_OUT_MON1                          ; 2b
	const BATTLE_BG_EFFECT_BETA_SEND_OUT_MON2                          ; 2c
	const BATTLE_BG_EFFECT_FLAIL                                       ; 2d
	const BATTLE_BG_EFFECT_BETA_PURSUIT                                ; 2e
	const BATTLE_BG_EFFECT_ROLLOUT                                     ; 2f
	const BATTLE_BG_EFFECT_VITAL_THROW                                 ; 30
	const BATTLE_BG_EFFECT_START_WATER                                 ; 31
	const BATTLE_BG_EFFECT_WATER                                       ; 32
	const BATTLE_BG_EFFECT_END_WATER                                   ; 33
	const BATTLE_BG_EFFECT_VIBRATE_MON                                 ; 34
	const BATTLE_BG_EFFECT_WOBBLE_PLAYER                               ; 35
	const BATTLE_BG_EFFECT_WOBBLE_SCREEN                               ; 36
